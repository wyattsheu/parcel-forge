...
def main():
 p=argparse.ArgumentParser();p.add_argument('--input',required=True);p.add_argument('--out',required=True);p.add_argument('--model',required=True);a=p.parse_args();out=Path(a.out);out.mkdir(parents=True,exist_ok=True);root=Path(__file__).resolve().parents[2];sys.path.insert(0,str(root/'external/TripoSR'))
