...
def main():
 ...
 try:
  ...
  visual=surface(asset,'/Drill/Visual');visual.CreateDisplayColorPrimvar(UsdGeom.Tokens.vertex).Set([Gf.Vec3f(*c) for c in colors]);collision=surface(asset,'/Drill/Collision');collision.CreatePurposeAttr('guide');UsdPhysics.CollisionAPI.Apply(collision.GetPrim());UsdPhysics.MeshCollisionAPI.Apply(collision.GetPrim()).CreateApproximationAttr('convexDecomposition');PhysxSchema.PhysxCollisionAPI.Apply(collision.GetPrim()).CreateContactOffsetAttr(.001)
