...
| S3 Parameterisation & static validation | JSON schema, dimension/thickness rules, USD read-back, 10 cases | Input-to-output hashes, dimensions, error classification | todo — card at docs/tasks/S3.md |
...
Do not start S3 until `./scripts/pf box --all` reproduces all three verdicts on a
fresh run and `docs/STATE.md` still matches the newest runs under `runs/`.
